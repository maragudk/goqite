...
// inTx runs the callback in a transaction, at the isolation level that suits the queue's SQL flavor.
func (q *Queue) inTx(ctx context.Context, cb func(*sql.Tx) error) error {
	...
	case SQLFlavorPostgreSQL:
		// Read committed is what the receive query is written for: it claims its row with
		// "for update skip locked" rather than relying on isolation. Pinning the level keeps that pairing
		// intact, where nil options would inherit the server default, which may be raised to serializable
		// and turn ordinary queue contention into serialization failures.
		opts = &sql.TxOptions{Isolation: sql.LevelReadCommitted}
	}

	return internalsql.InTx(ctx, q.db, opts, cb)
}
